...
* **Keep it concise.** A few sentences to a short paragraph is usually right — long enough to cover the skill's scope, short enough that it doesn't bloat the agent's context across many skills. The [specification](specification.md) enforces a hard limit of 1024 characters.
...
2. Verify the description is under the 1024-character limit (see [specification.md](specification.md)).
...
Once your skill triggers reliably, you'll want to evaluate whether it produces good outputs. See [Evaluating skill output quality](https://agentskills.io/skill-creation/evaluating-skills) for how to set up test cases, grade results, and iterate.

...
Use tmux, not `&`, `nohup`, `disown`, or fire-and-forget spawning. Resolve `scripts/resolve-manager-session.sh` relative to the directory containing this file.

```bash
eval "$("<absolute-directory-containing-this-file>/scripts/resolve-manager-session.sh" --shell)"
